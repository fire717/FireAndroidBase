...
//                __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","confidence %f", confidence);
...
int Yolo::load(const char *modeltype, int _target_size, const float *_norm_vals, bool use_gpu) {
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "yolov7-tiny-sim-opt.param");
    sprintf(modelpath, "yolov7-tiny-sim-opt.bin");
...
int Yolo::load(AAssetManager *mgr, const char *modeltype, int _target_size, const float *_norm_vals,
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "best.torchscript.ncnn.param");
    sprintf(modelpath, "best.torchscript.ncnn.bin");
...
int Yolo::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
    ...
    ncnn::Mat in = ncnn::Mat::from_pixels_resize(rgb.data, ncnn::Mat::PIXEL_RGB, img_w, img_h, w,
                                                 h);
    __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","target_size %d %d", w,h);
    // pad to target_size rectangle
    int wpad = (w + max_stride - 1) / max_stride * max_stride - w;
    int hpad = (h + max_stride - 1) / max_stride * max_stride - h;
    __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","pad %d %d", wpad,hpad);
    ncnn::Mat in_pad;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","8 proposals.size %d", proposals.size());
    // stride 16
    {
        ncnn::Mat out;
        ex.extract("out1", out);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","16 proposals.size %d", proposals.size());
    // stride 32
    {
        ncnn::Mat out;
        ex.extract("out2", out);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov7cpp","32 proposals.size %d", proposals.size());
